...
完成了将前端硬编码的报表模板迁移到后端数据库驱动的重大重构。

- **数据库建模**: 在 `models.py` 中使用SQLAlchemy为项目、报表定义、模板字段和指标创建了数据模型。
- **数据库迁移**: 初始化并配置了Alembic，成功生成并应用了数据库迁移脚本，在PostgreSQL数据库中创建了新表。
- **数据填充**: 创建了可重复执行的数据填充脚本 `seed.py`，将前端 `templates/*.js` 中的所有静态数据（包括报表结构、指标、字段、计算公式等）导入数据库。
- **API开发**: 在 `main.py` 中创建了新的API端点 `GET /api/reports/{report_id}/template`，用于向前端动态提供报表结构。


## 2025年9月7日：前端代码清理

在后端API和数据库迁移完成后，对前端代码进行了清理，移除了所有已迁移的、不再需要的静态数据文件和引用。

- **删除作废模板文件**: 删除了 `frontend/src/projects/heating_plan_2025-2026/templates/` 目录下的 `groupTemplate.js` 和 `subsidiaryTemplate.js` 文件。
- **删除作废用户数据文件**: 删除了 `frontend/src/auth.js` 文件，其中包含了现已迁移到数据库的硬编码用户列表。
- **重构菜单配置**: 编辑了 `frontend/src/projects/heating_plan_2025-2026/menu.js` 文件，移除了所有对已删除模板文件的 `import` 语句以及在报表定义中使用的 `template` 属性，使菜单配置与模板实现完全解耦。
