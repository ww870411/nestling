...
### 2025-09-06 11:15:00 (UTC+8)

*   **模块:** `views` (UI/视图层)
*   **修改内容:**
    1.  **Bug修复:** 修正了 `DashboardView.vue` 中的 `allReports` 计算属性，使其在生成报表列表前，先使用 `authStore` 中的 `accessibleUnits` 对 `menuData` 进行过滤。
*   **修改原因:**
    *   权限系统上线后，仅对左侧菜单栏进行了过滤，而首页的报表中心忘记了应用权限过滤，导致任何用户都能看到并访问所有报表，造成了权限漏洞。
*   **实现效果:**
    *   修复了权限漏洞，确保了首页报表中心和左侧菜单栏的权限控制逻辑一致，用户现在只能看到他们有权访问的报表。
